...
class Shop extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        ...
        $this->_postCollection = $this->_objectManager->create('Magefan\Stories\Model\Post')
            ...
            ->addFieldToFilter('is_displayed_shop', array('neq' => 1))
        ...
        if($counts<=10)
        {
            ...
            {
                if(++$counts>10) break;
                $item->setData('is_displayed_shop',1);
                $item->save();
                if($item->getTitle()!="promotions") 
                {
                    $idhtml=$idhtml.'<div class="shop-card-parent"><a class="card" href="'.$item->getPostUrl().'">
                             <img class="card-img-top img-fluid" src="'.$helper->init($item->getImage())->resize(390).'" alt="Card image cap" width="100%">
                           </a></div>';
                }
                else 
                {
                    $idhtml=$idhtml.'<div class="shop-card-parent"><a class="card" href="'.$item->getContentHeading().'">
                             <img class="card-img-top img-fluid" src="'.$helper->init($item->getImage())->resize(390).'" alt="Card image cap" width="100%">
                           </a></div>';
                }
                if($post_count>=10)
